...
getdata();
async function getdata(lineOffset = 0) {
   ...
   while (offset < total) {
        ...
        // let url = 'kinneret_level.csv';
        const response = await fetch(url);
        const json = await response.json();
        //.json();
        //let records = json.split('\n');
        ...
        //     let col = elem.split(',');
        //     let date = col[0].split('/');
        //     xs.push(date[date.length - 1]);
        //     ys.push(parseFloat(col[1])); 
